...
import { useParametrizacaoAlertas, useDeleteParametrizacaoAlerta } from '../hooks/useParametrizacaoAlertas'

const TABLE_HEADERS = ['ID', 'Insumo', 'Qtd Mínima', 'Qtd Máxima', 'Dias Vencimento', 'Ações']
...
export default function ParametrizacaoAlertaListPage() {
  const navigate = useNavigate()
  const [deleteTarget, setDeleteTarget] = useState<{ id: number } | null>(null)

  const { data: parametrizacoes, isLoading } = useParametrizacaoAlertas()
  const deleteMutation = useDeleteParametrizacaoAlerta()
  ...
  return (
    <div>
      <PageHeader title="Parametrização de Alertas" subtitle="Configure limites de estoque e vencimento por insumo">
        <button
          onClick={() => navigate('/estoque-insumos/parametrizacao-alertas/nova')}
        ...
        {isLoading ? <SkeletonRows /> : (
          (parametrizacoes ?? []).map((p) => (
            <tr key={p.id} className="hover:bg-gray-50 transition-colors">
              ...
              <td className="px-4 py-3 font-medium text-gray-900">{p.insumoNome ?? `#${p.insumoId}`}</td>
              <td className="px-4 py-3 text-gray-600">{p.quantidadeMinimaEstoque}</td>
              <td className="px-4 py-3 text-gray-600">{p.quantidadeMaximaEstoque}</td>
              <td className="px-4 py-3 text-gray-600">{p.quantidadeDiasVencimento} dias</td>
              <td className="px-4 py-3">
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => navigate(`/estoque-insumos/parametrizacao-alertas/${p.id}/editar`)}
