...
import { useAlertas, useResolverAlerta, useVerificarAlertas } from '../hooks/useAlertas'

const TABLE_HEADERS = ['ID', 'Tipo', 'Insumo', 'Qtd Atual', 'Referência', 'Data', 'Status', 'Ações']
...
const TIPO_LABELS: Record<number, { label: string; color: string }> = {
  ...
  3: { label: 'Estoque Máximo', color: 'bg-yellow-100 text-yellow-700' },
}
...
export default function AlertaListPage() {
  ...
  const { data, isLoading } = useAlertas(page, pageSize, Object.keys(filterParams).length > 0 ? filterParams : undefined)
  const resolverMutation = useResolverAlerta()
  const verificarMutation = useVerificarAlertas()
  ...
  return (
    <div>
      <PageHeader title="Alertas" subtitle="Monitore vencimentos e níveis de estoque">
        <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                >
                  ...
                  <option value="3">Estoque Máximo</option>
                </select>
              </div>
            </div>
        ...
        {alertas.map((a) => {
          ...
          return (
            <tr key={a.id} className="hover:bg-gray-50 transition-colors">
              ...
              <td className="px-4 py-3 font-medium text-gray-900">{a.insumoNome ?? `#${a.insumoId}`}</td>
