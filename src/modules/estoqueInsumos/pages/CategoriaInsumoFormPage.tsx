...
import { useCategoriaInsumo, useCreateCategoriaInsumo, useUpdateCategoriaInsumo } from '../hooks/useCategoriasInsumo'
...
export default function CategoriaInsumoFormPage() {
  const navigate = useNavigate()
  const { id } = useParams<{ id: string }>()
  const isEditing = !!id
  const numericId = Number(id ?? 0)

  const { data: categoria, isLoading } = useCategoriaInsumo(numericId)
  ...
  function handleSubmit(e: FormEvent) {
    ...
    if (isEditing) {
      updateMutation.mutate(
        ...
        { onSuccess: () => navigate('/estoque-insumos/categorias') },
      )
    } else {
