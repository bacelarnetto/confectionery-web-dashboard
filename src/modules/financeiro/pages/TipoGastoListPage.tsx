...
import { useTiposGasto, useDeleteTipoGasto } from '../hooks/useFinanceiro'
...
export default function TipoGastoListPage() {
  ...
  return (
    <div>
      <PageHeader title="Tipos de Gasto" subtitle="Categorize os gastos da confeitaria">
        <button
          onClick={() => navigate('/financeiro/tipos-gasto/novo')}
      ...
      <PageableTable
        ...
        isLoading={isLoading}
        isEmpty={!isLoading && tipos.length === 0}
        page={page}
      ...
      >
        {tipos.map((t) => (
          <tr key={t.id} className="hover:bg-gray-50 transition-colors">
            <td className="px-4 py-3 text-gray-500 text-sm">{t.id}</td>
            <td className="px-4 py-3 font-medium text-gray-900">{t.nome}</td>
            <td className="px-4 py-3">
              <div className="flex items-center gap-2">
                <button
                  onClick={() => navigate(`/financeiro/tipos-gasto/${t.id}/editar`)}
      ...
      <DeleteConfirmModal
        ...
        itemName={`Tipo de gasto "${deleteTarget?.nome}"`}
