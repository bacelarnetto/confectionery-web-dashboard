...
const TABLE_HEADERS = ['ID', 'Nome', 'Telefone', 'E-mail', 'Ações']
...
export default function ColaboradorListPage() {
  ...
  const { data, isLoading } = useColaboradores(page, pageSize, debouncedNome || undefined)
  const deleteMutation = useDeleteColaborador()

  const colaboradores = data?.content ?? []
  ...
  return (
    <div>
      <PageHeader title="Colaboradores" subtitle="Atendentes que acompanham/entregam itens de apoio nos eventos">
        <button
          onClick={() => navigate('/vendas/colaboradores/novo')}
      ...
      >
        {colaboradores.map((c) => (
          <tr key={c.id} className="hover:bg-gray-50 transition-colors">
            ...
            <td className="px-4 py-3 text-gray-600">{c.telefoneCelular}</td>
            <td className="px-4 py-3 text-gray-600">{c.email ?? '—'}</td>
            <td className="px-4 py-3">
              <div className="flex items-center gap-2">
                <button
                  onClick={() => navigate(`/vendas/colaboradores/${c.id}/editar`)}
      ...
      <DeleteConfirmModal
        ...
        itemName={`Colaborador "${deleteTarget?.nome}"`}
