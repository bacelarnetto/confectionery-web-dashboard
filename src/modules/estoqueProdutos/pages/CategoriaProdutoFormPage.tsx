...
import { useCategoriaProduto, useCreateCategoriaProduto, useUpdateCategoriaProduto } from '../hooks/useCategoriasProduto'
...
export default function CategoriaProdutoFormPage() {
  const navigate = useNavigate()
  const { id } = useParams<{ id: string }>()
  const isEditing = !!id
  const numericId = Number(id ?? 0)

  const { data: categoria, isLoading } = useCategoriaProduto(numericId)
  ...
  return (
    <div className="max-w-3xl">
      <PageHeader
        ...
        subtitle={isEditing ? 'Atualize os dados da categoria' : 'Cadastre uma nova categoria de produto'}
        backTo="/estoque-produtos/categorias"
      />
