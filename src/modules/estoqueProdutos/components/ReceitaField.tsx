...
import { Search, X, ChefHat, Clock, ChevronDown, ExternalLink } from 'lucide-react'
import { useReceita, useReceitas } from '../hooks/useReceitas'
import { useDebounce } from '../../../hooks/useDebounce'

interface ReceitaFieldProps {
  value: number
  onChange: (receitaId: number) => void
  disabled?: boolean
  required?: boolean
  className?: string
  placeholder?: string
...
export default function ReceitaField({
  value,
  onChange,
  disabled = false,
  required = false,
  className = '',
  placeholder = 'Buscar receita por nome (ex: Bolo de Cenoura)...',
}: ReceitaFieldProps) {
  ...
  // Busca a receita atualmente selecionada (quando houver id selecionado)
  const { data: receitaAtual, isLoading: loadingAtual } = useReceita(value)

  // Busca dinamicamente receitas pelo backend com paginação e filtro por nome
  const { data: resultsData, isLoading: loadingResults } = useReceitas(
    0,
    RESULT_LIMIT,
  ...
  // Reseta índice destacado ao mudar resultados ou busca
  useEffect(() => {
    setHighlightedIndex(-1)
  }, [resultsData, search])

  // Rola o item destacado para a visão se navegar via teclado
  useEffect(() => {
        ...
        <div className="flex items-center justify-between gap-3 p-3 text-sm border border-amber-200 bg-amber-50/40 rounded-xl transition-all">
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <div className="font-semibold text-gray-900 truncate">
                {receitaAtual?.nome ?? (loadingAtual ? 'Carregando receita...' : `Receita #${value}`)}
              </div>
              <div className="flex items-center gap-2 text-xs text-gray-500 mt-0.5">
                {receitaAtual?.ingredientes && (
                  <span className="font-medium text-amber-800 bg-amber-100/70 px-1.5 py-0.5 rounded">
                    {receitaAtual.ingredientes.length} {receitaAtual.ingredientes.length === 1 ? 'ingrediente' : 'ingredientes'}
                  </span>
                )}
                {receitaAtual?.tempoPreparo && (
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button
              ...
              disabled={disabled}
              onClick={handleClear}
              title="Remover receita selecionada"
            ...
            {loadingResults ? (
              ...
              <li className="px-4 py-4 text-xs text-center space-y-2">
                <p className="text-gray-500">
                  {search ? (
                    <>
                      Nenhuma receita encontrada para <span className="font-semibold text-gray-700">"{search}"</span>.
                    </>
                  ) : (
                    'Nenhuma receita cadastrada no sistema.'
                  )}
                </p>
                <a
                  href="/estoque-produtos/receitas/nova"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1 text-amber-700 hover:text-amber-800 font-medium hover:underline text-xs"
                >
                  Cadastrar nova receita <ExternalLink size={12} />
                </a>
              </li>
            ) : (
              results.map((r, index) => {
